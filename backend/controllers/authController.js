...
exports.forgotPassword = catchAsyncError( async (req,res,next)=>{
    ...
    try{
        sendEmail({
            email: user.email,
            subject: "oscart password recovery",
            message
        })
...
exports.changePassword = catchAsyncError(async(req,res,next)=>{
    ...
    if(!await user.isValidPassword(req.body.oldPassword)){
        return next(new ErrorHandler('Old Passord is Incorrect',401))
    }
...
exports.getUser = catchAsyncError(async(req,res,next)=>{
    ...
    if(!user){
        return next(new ErrorHandler(`User not found ith this id ${req.params.id}`))
    }
    res.status(200).json({
        success: true,
        user
